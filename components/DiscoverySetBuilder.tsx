...
import { ShoppingBag, Sparkles, X, Plus, Bell, MessageCircle } from "lucide-react";
      ...
      <section className="relative overflow-hidden bg-background pt-16 text-foreground md:min-h-screen md:pt-24">
        ...
        <div className="relative mx-auto grid w-full max-w-[1280px] items-center gap-7 px-4 py-6 sm:px-6 md:min-h-[calc(100vh-5rem)] md:gap-10 md:py-10 lg:grid-cols-[0.72fr_1.28fr] lg:px-8 lg:py-8">
          ...
          >
            ...
            {isDiscoverySetSoldOut && (
              <div className="mt-5 rounded-[1.4rem] border border-[#25D366]/35 bg-[linear-gradient(135deg,#f6fcf8,#eef9f2)] p-4 sm:p-5 text-stone-900 shadow-sm">
                <div className="flex items-center justify-between gap-3 mb-2">
                  <div className="flex items-center gap-2 font-bold text-xs uppercase tracking-wider text-[#128C7E] font-sans">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#25D366]/20 text-[#128C7E]">
                      <MessageCircle className="h-4 w-4" />
                    </span>
                    <span>Discovery Set Sold Out - WhatsApp Alert</span>
                  </div>
                  <span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-800 font-sans">
                    WhatsApp
                  </span>
                </div>

                <p className="text-xs text-stone-600 font-sans mb-3.5 leading-relaxed">
                  Get notified on WhatsApp as soon as the Discovery Set is back in stock.
                </p>
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-emerald-600/25 bg-white px-3.5 py-2.5 text-xs font-sans text-stone-900 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-[#128C7E]"
                  />
                  <button
                    type="button"
                    onClick={() => handleNotifySubmit()}
                    disabled={isNotifySaving}
                    className="rounded-xl bg-[#128C7E] px-4 py-2.5 text-xs font-bold font-sans uppercase tracking-wider text-white hover:bg-[#0e6f64] disabled:opacity-50 transition shadow-xs"
                  >
                    {isNotifySaving ? "Saving..." : "Notify Me"}
                  </button>
                </div>

                <div className="mt-3.5 flex items-center justify-between border-t border-emerald-600/15 pt-2.5 text-[11px] text-stone-600 font-sans">
                  <span>Or ask us directly:</span>
                  <a
                    href="https://wa.me/919559024822?text=Hi%20HUME,%20please%20notify%20me%20when%20the%20Discovery%20Set%20is%20back%20in%20stock!"
                    target="_blank"
                    rel="noreferrer"

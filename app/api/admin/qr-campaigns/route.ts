...
export async function GET(req: Request) {
  try {
    ...
    if (id) {
      ...
      // Fetch specific events linked to this QR ID or city & target page
      const events = await db
        .select()
        .from(flyerCampaignEvents)
        .where(
          sql`qr_id = ${id} or (city = ${campaign.city} and target_page = ${campaign.targetPage})`
        )
        .orderBy(desc(flyerCampaignEvents.createdAt))
        .limit(100);

      // Compute exact scan count and last scanned date for this campaign
    ...
    // List all campaigns enriched with scan event counts
    const campaigns = await db
      .select()
      .from(qrCampaigns)
      .orderBy(desc(qrCampaigns.createdAt));

    // Fetch total scans grouped by city & target_page or qr_id
    ...
    const enrichedCampaigns = campaigns.map((c) => {
      const matchingScans = scanEvents.filter(
        (e) => e.qrId === c.id || (e.city === c.city && e.targetPage === c.targetPage)
      );

...
const initialItems: StockItem[] = [
  { id: "oil-imagine", name: "Imagine fragrance oil", sku: "OIL-IMG", group: "Fragrance Oil", unit: "g", onHand: 860, note: "Main production concentrate" },
  { id: "oil-ombre", name: "Ombre Leather fragrance oil", sku: "OIL-OMB", group: "Fragrance Oil", unit: "g", onHand: 420, note: "Check incoming batch" },
  { id: "oil-sauvage", name: "Sauvage fragrance oil", sku: "OIL-SAV", group: "Fragrance Oil", unit: "g", onHand: 9, note: "Reorder soon" },
  { id: "sample-imagine", name: "Imagine sample oil", sku: "SMP-IMG", group: "Sample Oil", unit: "g", onHand: 96, note: "Allocated for 3ml testers" },
  { id: "sample-ombre", name: "Ombre Leather sample oil", sku: "SMP-OMB", group: "Sample Oil", unit: "g", onHand: 8, note: "Allocated for Discovery Set" },
    ...
    <>
      <div className="relative overflow-hidden rounded-3xl border border-emerald-300/15 bg-[radial-gradient(circle_at_82%_10%,rgba(52,211,153,.16),transparent_28%),linear-gradient(125deg,#0d1713_0%,#10271e_55%,#0b1712_100%)] p-5 shadow-[0_24px_70px_rgba(0,0,0,.24)] sm:p-7">
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.22em] text-emerald-200/55"><Warehouse className="h-4 w-4" /> Inventory command centre</div><h2 className="mt-3 text-2xl font-semibold text-white sm:text-3xl">Know what is on hand before production starts.</h2><p className="mt-2 text-sm leading-6 text-white/45">Track oils by gram and packaging by piece, with an automatic red alert when any item reaches 10 or below.</p></div>
      ...
      <section className="overflow-hidden rounded-3xl border border-white/8 bg-white/[0.02]">
        <div className="flex flex-col gap-4 border-b border-white/8 p-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex gap-2 overflow-x-auto pb-1">
            {tabs.map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`shrink-0 rounded-full border px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.12em] ${activeTab === tab ? "border-white bg-white text-black" : "border-white/10 text-white/45 hover:text-white"}`}>{tab}</button>)}
          </div>
            ...
            <tbody>
              {visibleItems.map((item) => {
                ...
                return <tr key={item.id} className="group align-middle shadow-[0_8px_24px_rgba(0,0,0,.08)]">
                  <td className={`${cellClass} rounded-l-2xl border-l px-4`}><div className="flex items-center gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${status.alert ? "border-rose-400/20 bg-rose-400/10 text-rose-300" : "border-emerald-300/10 bg-emerald-300/[0.07] text-emerald-300/70"}`}><GroupIcon className="h-4 w-4" /></div><div><input value={item.name} onChange={(event) => updateItem(item.id, "name", event.target.value)} className="block w-52 bg-transparent text-sm font-semibold text-white outline-none transition focus:text-emerald-100" /><div className="mt-1 flex items-center gap-2"><span className="h-px w-3 bg-white/15" /><input value={item.sku} onChange={(event) => updateItem(item.id, "sku", event.target.value.toUpperCase())} className="block w-36 bg-transparent font-mono text-[9px] uppercase tracking-[.1em] text-white/30 outline-none focus:text-white/60" /></div></div></div></td>
                  <td className={cellClass}><select value={item.group} onChange={(event) => { const category = categories.find((entry) => entry.name === event.target.value); if (!category) return; updateItem(item.id, "group", category.name); updateItem(item.id, "unit", category.unit); }} className="h-10 rounded-xl border border-white/10 bg-black/25 px-3 text-xs font-medium text-white/65 outline-none transition hover:border-white/20 focus:border-emerald-300/35">{categories.map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select></td>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 p-4 backdrop-blur-md" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowCategoryManager(false); }}>
          <div role="dialog" aria-modal="true" aria-labelledby="category-manager-title" className="w-full max-w-lg overflow-hidden rounded-3xl border border-emerald-300/15 bg-[#101713] shadow-[0_30px_100px_rgba(0,0,0,.7)]">
            ...
            <div className="space-y-5 p-5 sm:p-6">
              ...
              <div><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-white/35">Your categories</p><span className="text-[10px] text-white/25">{categories.length} total</span></div><div className="max-h-64 space-y-2 overflow-y-auto pr-1">{categories.map((category) => { const assignedCount = items.filter((item) => item.group === category.name).length; return <div key={category.id} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.025] p-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-300/[0.07] text-emerald-300/70">{category.unit === "pcs" ? <Boxes className="h-4 w-4" /> : <FlaskConical className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-white">{category.name}</p><p className="mt-0.5 text-[10px] text-white/30">{category.unit === "pcs" ? "Measured in pieces" : "Measured in grams"} • {assignedCount} item{assignedCount === 1 ? "" : "s"}</p></div><button type="button" onClick={() => deleteCategory(category)} className={`flex h-9 w-9 items-center justify-center rounded-xl border transition ${assignedCount ? "cursor-not-allowed border-white/5 text-white/15" : "border-rose-400/15 text-rose-300/65 hover:border-rose-400/35 hover:bg-rose-400/10 hover:text-rose-200"}`} aria-label={`Delete ${category.name}`} title={assignedCount ? "Move or delete assigned items first" : `Delete ${category.name}`}><Trash2 className="h-4 w-4" /></button></div>; })}</div></div>
